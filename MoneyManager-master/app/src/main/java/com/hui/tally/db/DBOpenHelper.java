...
public class DBOpenHelper extends SQLiteOpenHelper {
    public DBOpenHelper(@Nullable Context context,String name,SQLiteDatabase.CursorFactory factory,int version) {
...
//    创建数据库的方法，只有项目第一次运行时，会被调用
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        //创建待办表todotb
        sql = "create table todotb(id integer primary key autoincrement,todoName varchar(10),year integer,month integer,day integer,money float)";
        db.execSQL(sql);
        //创建心愿表wishtb
        sql = "create table wishtb(id integer primary key autoincrement,wishName varchar(10),totalMoney float,curMoney float,perMoney float,cycleDay integer,done integer,date varchar(80))";
        db.execSQL(sql);
    }


    private void insertType(SQLiteDatabase db) {
//      向typetb表当中插入元素，相当于初始化类型

...
public class OutcomChartFragment extends BaseChartFragment {
    ...
    protected void setAxisData(int year, int month) {
        ...
        if (list.size() == 0) {//如果这个月没有流水
            ...
            for (int i = 0; i < 31; i++) {//图上最多有31条柱子
                //初始化每一根柱子，添加到柱状图当中
                BarEntry entry = new BarEntry(i, 0.0f);//高度先默认为0
                barEntries1.add(entry);
            }
            for (int i = 0; i < list.size(); i++) {
                BarChartItemBean itemBean = list.get(i);
                int day = itemBean.getDay();   //获取该消费的日期
                //根据天数和消费money，获取x轴的位置，然后设置y轴高度
                int xIndex = day-1;
